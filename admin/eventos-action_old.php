...
if(isset($_POST['acao']) && $_POST['acao'] == "cadastra")
{
    ...
    else
    {
        ...
        $rs = mysql_query("INSERT INTO eventos VALUES('','$nm_evento','$texto','$local','$dt_evento','$tempo_duracao','1','".time()."')");
        $cd = mysql_insert_id();
            ...
            if(!empty($_FILES['foto']['name'][$x]))
            {
                ...
                if($ext == "jpg" || $ext == "gif" || $ext = "png")
                {
                    ...
                    if($tam_x > $tam_y)
                    {
                        ...
                        reduz_imagem($_FILES['foto']['tmp_name'][$x], 600, 450, $ft_grande);
                    }
                    else
                    {
                        reduz_imagem($_FILES['foto']['tmp_name'][$x], 75, 57, $thumb);

                        reduz_imagem($_FILES['foto']['tmp_name'][$x], 450, 338, $ft_grande);
                    }
                    ...
                    $res1 = mysql_query("INSERT INTO fotos_eventos VALUES('','$cd','$thumbbd','$grandebd','$originalbd','1')") or die(mysql_error());
                    $prim++;
                }
            }
        }
...
elseif(isset($_POST['acao']) && $_POST['acao'] == "edita")
{
    ...
    else
    {
        ...
        $rs = mysql_query("UPDATE eventos SET nm_evento='$nm_evento', descricao='$texto', local='$local', dt_evento='$dt_evento', tempo_duracao='$tempo_duracao' WHERE cd_evento='$cd'");
...
elseif(isset($_POST['acao']) && $_POST['acao'] == "gerencia")
{
    ...
    $rs = mysql_query("SELECT * FROM fotos_eventos WHERE cd_evento='$cd' ORDER BY cd_foto ASC");
    while($var = mysql_fetch_array($rs))
    {
        $valor = $var['cd_foto'];
        $rs1 = mysql_query("UPDATE fotos_eventos SET ativo='".$$valor."' WHERE cd_foto='".$valor."'");
    }
    ...
    if(count($apagar))
    {
        foreach($apagar as $item)
        {
            ...
            $rs = mysql_query("SELECT * FROM fotos_eventos WHERE cd_foto='$item'");
            $ft = mysql_fetch_array($rs);
            @unlink("../".$ft['caminho_thumb']);
            @unlink("../".$ft['caminho_foto']);
            //@unlink("../".$ft['caminho_original']);

            $rs = mysql_query("DELETE FROM fotos_eventos WHERE cd_foto='$item'");

            //$rs = mysql_query("UPDATE fotos_eventos SET ativo='0' WHERE cd_foto='$item'");
        }
