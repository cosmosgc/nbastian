...
<div id="photoStrip">
    <img src="imagens/foto.gif" alt="" width="600" height="400" />
</div>
<?php
$cd = isset($_GET['cd']) ? intval($_GET['cd']) : "";
$cd_foto = isset($_GET['cd_foto']) ? intval($_GET['cd_foto']) : "";

$rs1 = mysql_query("SELECT cd_categoria, vl_foto FROM galerias WHERE cd_galeria='$cd'");
list($cd_cat, $vl_foto) = mysql_fetch_array($rs1);

$rs1 = mysql_query("SELECT venda FROM categorias WHERE cd_categoria='$cd_cat'");
list($pode_vender) = mysql_fetch_array($rs1);


$rs = mysql_query("SELECT * FROM fotos_galeria WHERE cd_foto='$cd_foto' AND ativo='1' ORDER BY cd_foto ASC");
...
<p style="text-align: left; margin-left: 25px; margin-right: 25px;">
<span class="left">
<?php
if($pode_vender && $vl_foto > 0)
{
    echo '<strong>Voc&ecirc; pode comprar essa foto por R$ '.number_format($vl_foto, 2, ',','.').'.</strong> <br />Adicione-a ao carrinho ou clique para ver  todas as fotos adicionadas.';
}
else
{
...
if($pode_vender && $vl_foto > 0)
      {
            echo '<a onclick="adicionaImagem(\''.$cd_foto.'\');" href="javascript:void(0);">
